<html>
    <head>
        <title>SUPOO SERVER</title>
        <style>
            form {
            border-width:5px;
            border-style:solid;
            border-color:red;
}
        </style>
    </head>
    <body bgcolor="">
        
    <legend align="center" bgcolor="red">
    <a href="{{ url('/getHome') }}" class="text-sm text-gray-700 dark:text-gray-500 underline">Home</a> | 
    <a href="{{ url('/getContact') }}" class="text-sm text-gray-700 dark:text-gray-500 underline">Contact Us</a> |
    <a href="{{ url('/getAbout') }}" class="text-sm text-gray-700 dark:text-gray-500 underline">About Us</a> 
        <h1 style="color:red;">Super King</h1>
        <h3>Home</h3>
        <legend align="center" >
        <form action="getHome" method="POST">
            @csrf
            <fieldset style="color:blue;">
                <Address>
                  NAME :  <input type="text" name="Name" placeholder="enter name"></input><br><br>
                  E-MAIL :  <input type="email" name="email" placeholder="enter E-mail"></input><br><br>
                  DOB :  <input type="date" name="DOB" ></input><br><br>
                  STATE : <input type="radio" name="state" id="AP"><label for="AP">AndhraPradesh</label></input>
                  <input type="radio" name="state" id="TS"><label for="TS">Telagana</label></input><br><br>
                  CITY : <select name="city">
                    
                    <option>Vizag</option>
                    <option>Anakapalli</option>
                    <option>Srikakulam</option>
                    <option>Goa</option>
                    <option>Pune</option>
                  </select><br><br>

                Message :  <textarea rows="5" cols="50" name="message"></textarea><br><br>
                <input type="submit" name="submit" ></input><input type="reset" name="reset" ></input><br><br><br><br>
                  </address>
                  
                  
            </fieldset>
        </form>
</legend>
</legend>

    </body>
</html>